	...
	<div id="adminuserdetail" class="adminuserdetail">			
		<table class="layui-table">
		    <tbody>
		      ...
		      <tr>
		        <td class="tdbck">权限名称</td>
		        <td><span id="txtadminuserrealname"></span></td>
		      </tr>
		      <tr>
		        <td class="tdbck">包含权限</td>
		...
		<blockquote class="layui-elem-quote not_border_left">
			<form class="layui-form" action="">
			  	<div class="layui-input-inline">
					<input type="text" name="userName" id="userName" placeholder="请输入权限名称" class="layui-input" autocomplete="off">
			    </div>
			    <div class="layui-inline">
	     	   		<button id="btnselfrontinfo" type="button" class="layui-btn layui-bg-blue">查询</button>
			    </div>
				<button type="button" class="layui-btn layui-bg-blue" id="addartType" lay-event="addartType" lay-filter="addartType" style="margin-left: 10px;">新增权限</button>
			</form>
		</blockquote>
		<!-- 条件筛选框End -->
					...
					<div class="layui-form-item">
						...
						<div class="layui-input-block">
							<input type="text" name="addUserName" id="addUserName"
								lay-verify="addUserName" autocomplete="off" placeholder="请输入权限id" class="layui-input">
						</div>
					</div>
					<div class="layui-form-item">
						<label class="layui-form-label">权限名称:</label>
						<div class="layui-input-block">
							<input type="text" name="nickName" id="nickName" autocomplete="off" placeholder="请输入权限名称" class="layui-input">
						</div>
					</div>
	...
	layui.use([ 'table', 'form', 'layer', 'laydate', 'laytpl', 'element' ], function() {
		...
		$("#addartType").click(function(){
			...
			layer.open({
				...
				btn : [ '保存', '返回' ],
				yes : function() {
					var authorityid = $("#addUserName").val().trim();
					var authorityname = $("#nickName").val().trim();
					var includeid = $("#usertype").val().trim();
					...
					$.ajax({
						type : 'get',
						url : '../RolePermission/addrolepermission?authorityid=' + authorityid + '&authorityname=' + authorityname+'&includeid='+includeid,
						datatype : 'json',
		...
		table.on('tool(blogUser)', function(obj) {
			var data = obj.data;
			$("#txtclaid").text(data.authorityid);
			$("#txtadminuserrealname").text(data.authorityname);
			$("#txtadminuserusertype").text(data.includeid);
				...
				case 'del':
					layer.confirm('确定要删除么？', {
					  ...
					  icon:3
						...
						$.ajax({
			        		type: 'get',
			        		url: "../RolePermission/deleterolepernisssion?authorityid=" + data.authorityid,
			        		dataType: 'json',
			        		success:function(data){
			        			if(data.code == 0){
			        				layer.confirm(data.msg, {
