...
<body>
	...
	layui.use([ 'table', 'form', 'layer', 'laydate', 'laytpl', 'element' ], function() {
		...
		$("#btnselfrontinfo").click(function(){
			...
			var selectid=$("#selectid").val().trim();
			var parm='?useridornickname='+useridornickname+'&selectid='+selectid;
		...
		$("#addartType").click(function(){
			...
			layer.open({
				...
				yes : function() {
					...
					$.ajax({
						type : 'get',
						url : '../addadminuser.action?userid=' + addUserName +'&realname='+realname+'&usertype='+usertype,
						datatype : 'json',
		...
		table.on('tool(blogUser)', function(obj) {
			...
			$("#txtadminuserdesc").text(data.description);
				...
				case 'del':
					layer.confirm('确定要删除么？', {
					  ...
					  icon:3
						...
						$.ajax({
			        		type: 'get',
			        		url: "../deleteadminuser.action?userid=" + data.userid,
			        		dataType: 'json',
			        		success:function(data){
			        			if(data.code == 0){
			        				layer.confirm(data.msg, {
